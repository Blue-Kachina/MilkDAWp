...
### Transition behavior in MilkDAWp (projectM playlists)
- You do not need to specify a transition type per preset change.
- MilkDAWp configures projectM’s global timing and transition knobs and uses the playlist API:
  - Soft cuts are the default (3 s crossfade).
  - If “Hard cut” is enabled under Auto-play, we set soft-cut duration to 0 and allow on‑beat hard jumps per projectM’s rules.
- Manual selection and initial playlist positioning default to soft unless Auto‑play Hard Cut is enabled.
- Auto progression is driven by projectM; we do not schedule transitions ourselves except for a light watchdog nudge if internal auto stalls.
